...
noise = [0,2.5,5,7.5,10,12.5,15,17.5,20]
...
chi_HH_k = np.zeros((len(noise),2))
chi_IF_k = np.zeros((len(noise),2))
chi_IZH_k = np.zeros((len(noise),2))
chi_ML_k = np.zeros((len(noise),2))

chi_rel_HH_k = np.zeros((len(noise),2))
chi_rel_IF_k = np.zeros((len(noise),2))
chi_rel_IZH_k = np.zeros((len(noise),2))
chi_rel_ML_k = np.zeros((len(noise),2))

rel_HH_k = np.zeros((len(noise),2))
rel_IF_k = np.zeros((len(noise),2))
rel_IZH_k = np.zeros((len(noise),2))
rel_ML_k = np.zeros((len(noise),2))


rossum_HH_k = np.zeros((len(noise),2))
rossum_IF_k = np.zeros((len(noise),2))
rossum_IZH_k = np.zeros((len(noise),2))
rossum_ML_k = np.zeros((len(noise),2))
...
firing_rate_HH_k = np.zeros(len(noise))
firing_rate_IF_k = np.zeros(len(noise))
firing_rate_IZH_k = np.zeros(len(noise))
firing_rate_ML_k = np.zeros(len(noise))
...
for j in range(0,len(noise)):
    ...
    for i in range(0,n_measurements):
        ...
        x1 = np.random.normal(0,noise[j],int(t_final/dt))
        x2 = np.random.normal(0,noise[j],int(t_final/dt))
        x3 = np.random.normal(0,1.5/5*noise[j],int(t_final/dt))
        x4 = np.random.normal(0,1.5/5*noise[j],int(t_final/dt))
        ...
        proc_HH = mp.Process(target = project.models.HH_Neuron_Pairs, args= (dt,t_final,order,[V0,V0],[n0,n0],[m0,m0],[h0,h0],*params_HH,I_HH,Is,0.012,tau,return_dict) )
        jobs.append(proc_HH)    
        proc_HH.start()

        proc_IF = mp.Process(target = project.models.LIF_Neuron_Pairs, args = (dt, t_final,order,y0,*params_LIF,I_IF,Is,0.012,tau,spikelet_fun(0.012),return_dict))
        jobs.append(proc_IF)
        proc_IF.start()
    
        proc_IZH = mp.Process(target = project.models.IZH_Neuron_Pairs, args =(dt,t_final,order,y0,u0,I_IZH,Is,*params_IZH,0.04,tau,return_dict))
        jobs.append(proc_IZH)
        proc_IZH.start()

        proc_ML = mp.Process(target= project.models.ML_Neuron_Pairs, args= (dt,t_final,2,y0,w0,*params_ML,I_ML,Is,0.008,tau,return_dict))
        jobs.append(proc_ML)
        proc_ML.start()
...
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/chi_HH_noise.txt',chi_HH_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/chi_IF_noise.txt',chi_IF_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/chi_IZH_noise.txt',chi_IZH_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/chi_ML_noise.txt',chi_ML_k)

np.savetxt('simulation_functions/saved_data/sync_noise_chemical/rel_HH_noise.txt',rel_HH_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/rel_IF_noise.txt',rel_IF_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/rel_IZH_noise.txt',rel_IZH_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/rel_ML_niose.txt',rel_ML_k)

np.savetxt('simulation_functions/saved_data/sync_noise_chemical/firing_rate_HH_noise.txt',firing_rate_HH_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/firing_rate_IF_noise.txt',firing_rate_IF_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/firing_rate_IZH_noise.txt',firing_rate_IZH_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/firing_rate_ML_noise.txt',firing_rate_ML_k)

np.savetxt('simulation_functions/saved_data/sync_noise_chemical/rossum_HH_noise.txt',rossum_HH_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/rossum_IF_noise.txt',rossum_IF_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/rossum_IZH_noise.txt',rossum_IZH_k)
np.savetxt('simulation_functions/saved_data/sync_noise_chemical/rossum_ML_noise.txt',rossum_ML_k)

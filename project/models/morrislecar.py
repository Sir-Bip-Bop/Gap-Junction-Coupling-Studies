...
def rk_ml_2_Rossum(dt,t_final,order,y0,w0,psi,V1,V2,V3,V4,gna,gk,gshunt,Ena,Ek,Eshunt,C,I,Isyn,strength,tau,E_matrix,C_matrix,return_dict):
    '''
    Runge-Kutta integration of the 4th order of the Morris-Lecar model
    '''
...
def rk_ml_2_scale_synaptic(dt,t_final,order,y0,w0,psi,V1,V2,V3,V4,gna,gk,gshunt,Ena,Ek,Eshunt,C,I,Isyn,strength,tau,E_matrix,C_matrix,return_dict):
    '''
    Runge-Kutta integration of the 4th order of the Morris-Lecar model
    '''
...
def rk_ml_scale_synaptic(dt,t_final,order,y0,w0,psi,V1,V2,V3,V4,gna,gk,gshunt,Ena,Ek,Eshunt,C,I,Isyn,strength,tau):
    '''
    Runge-Kutta integration of the 4th order of the Morris-Lecar model
    '''
    ...
    for i in range(0,Nsteps - 1):
        ...
        for k in range(0,num_neurons):
            ...
            if i > 0 and Y[i,k*(2+order)] > 10 and check[k] == 0:
                ...
                for l in range(0,num_neurons):
                    if l != k:
                        Y[i+1,l*(2+order) + 2 + order-1] = Y[i+1,l * (2+order) +2 + order-1] + Isyn[k,l] / num_connections[k]
            data[i+1,k] = Y[i+1,k*(2+order)]  
            if data[i+1,k] < 0:
                check[k] = 0

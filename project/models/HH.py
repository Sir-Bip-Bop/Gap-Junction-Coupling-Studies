...
def rk_simplemodel_Rossum_parallel(dt, t_final, order, y0, n0, m0, h0, gna, gk, gl, Ena, Ek, El, C, I, Isyn, strength, tau, return_dict):
    ''' 
    Runge Kutta integration of the 4th order of the HH model, for various orders and numbers of neurons
    '''
    Nsteps = int(t_final/dt)
...
def rk_HH_Rossum(dt, t_final, order, y0, n0, m0, h0, gna, gk, gl, Ena, Ek, El, C, I, Isyn, strength, tau, E_matrix, C_matrix, return_dict):
    ''' 
    Runge Kutta integration of the 4th order of the HH model, for various orders and numbers of neurons
    '''
...
def rk_simplemodel_scale_synaptic(dt, t_final, order, y0, n0, m0, h0, gna, gk, gl, Ena, Ek, El, C, I, Isyn, strength, tau):
    ''' 
    Runge Kutta integration of the 4th order of the HH model, for various orders and numbers of neurons
    '''
    Nsteps = int(t_final/dt)
    ...
    for i in range(0,Nsteps-1):
        ...
        for k in range(0,num_neurons):
            ...
            if i > 0 and Y[i,k*(4+order)] > 10 and check[k] == 0:
                ...
                for l in range(0,num_neurons):
                    if l != k:
                        Y[i+1,l*(4+order) + 4 + order-1] = Y[i+1,l * (4+order) +4 + order-1] + Isyn[k,l] /num_connections[k]
            data[i+1,k] = Y[i+1,k*(4+order)]  
            if data[i+1,k] < 0:
                check[k] = 0
...
def rk_HH_scale_synaptic(dt, t_final, order, y0, n0, m0, h0, gna, gk, gl, Ena, Ek, El, C, I, Isyn, strength, tau, E_matrix, C_matrix, return_dict):
    ''' 
    Runge Kutta integration of the 4th order of the HH model, for various orders and numbers of neurons
    '''

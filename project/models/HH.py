...
def HH_Equation_Network_test(y,n,m,h,synaptic,order,gna,gk,gl,Ena,Ek,El,C,I,tau,gap_junction,connectivity_matrix,gap_current,synaptic_current):
    ...
    Parameters:
        ...
        connectivity_matrix (dok_matrix):
            An sparse connectivity matrix, containing ones if the neurons are connected and 0 if the neurons are not connected
        gap_current (tuple[tuple[float,float]):
            Gap junction current for each cell
        synaptic_current (tuple[tuple[float,float]]):
...
def HH_Neuron_Network_tests(dt, t_final, order, y0, n0, m0, h0, gna, gk, gl, Ena, Ek, El, C, I, Isyn, gap_junction, tau, E_matrix, C_matrix, gap_current, synaptic_current, return_dict=0):
    ''' 
    Runge-Kutta integration of the 4th order of the HH model in the case of pairs of network of neurons, modified to track the values of all the relative currents
